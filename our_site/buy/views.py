...
def trans_info (request, pk):
    try:
        request_paper = Paper.objects.get(pk = pk)
    except Paper.DoesNotExist:
        return HttpResponse("the Paper you require is not exist")
    
    return render(request, 'buy/trans_info.html', {'paper' : request_paper})
...
def applicate_transact(request, pk):
    if request.method == 'POST':
        try:
            require_paper = Paper.objects.get(pk = pk)
        except Paper.DoesNotExist:
            return HttpResponse("the paper you require is not exist")
        
        check_transaction = Transaction.objects.filter(trans_composition = require_paper, trans_user = request.user.commuser_relation)
        ...
        new_transaction.trans_composition = require_paper
        new_transaction.trans_price = require_papers.price
...
def expert_trans_list(request):
    ...
    transaction_list = Transaction.objects.filter(trans_paper__expert = user)
